    ...
    <div className='mt-10 max-w-6xl mx-auto mb-5 px-4 md:px-0'>
      {
          ...
          <div>
            <div className='mt-10'>
              {cartItem.map((item, index) => {
                return <div key={index} className='bg-gray-100 dark:bg-gray-800 p-4 md:p-5 rounded-md flex flex-col sm:flex-row items-start sm:items-center justify-between mt-3 w-full gap-4'>
                  <div className='flex items-center gap-3 md:gap-4 w-full sm:w-auto'>
                    <img src={item.image} alt={item.title} className='w-16 h-16 md:w-20 md:h-20 rounded-md object-contain bg-white' />
                    <div className='flex-1 min-w-0'>
                      <h1 className='md:w-[300px] line-clamp-2 text-sm md:text-base dark:text-white'>{item.title}</h1>
                      <p className='text-red-500 font-semibold text-base md:text-lg'>${item.price}</p>
                    </div>
                  </div>
                  <div className='flex items-center justify-between w-full sm:w-auto gap-4'>
                    <div className='bg-red-500 text-white flex gap-4 p-2 rounded-md font-bold text-lg md:text-xl'>
                    ...
                    <span onClick={() => deleteItem(item.id)} className='hover:bg-white/60 dark:hover:bg-gray-700 transition-all rounded-full p-3 hover:shadow-2xl'>
        ...
        </div> : <div className='flex flex-col gap-3 justify-center items-center h-[400px] md:h-[600px] px-4'>
          <h1 className='text-red-500/80 font-bold text-2xl md:text-5xl text-center'>Oh no! Your cart is empty</h1>
          <img src={emptyCart} alt="Empty cart" className='w-[250px] md:w-[400px]' />
          <button onClick={() => navigate('/products')} className='bg-red-500 text-white px-4 py-2 rounded-md cursor-pointer hover:bg-red-600 transition-colors'>Continue Shopping</button>
        </div>
      }
    </div>
  )
